...
import { CronFormat, type ICronFormatter, type CronExpression, type CronFormatPart } from "../cron-format";
...
export class IdiomsCronFormat implements ICronFormatter {
  ...
  constructor(locale: string | Intl.Locale) {
    ...
    this.localeDictionary =
      idiomsLocales[this.localeString] ??
      idiomsLocales["en"]!;
  ...
  format(cronExpression: Cron | CronExpression): string {
    ...
    if (this.localeString === "es-CL-Flaite") {
      ...
      result = result.replace(/ en ([\wáéíóú]+) o ([\wáéíóú]+) o ([\wáéíóú]+)/, " en $1, $2 y $3,");
